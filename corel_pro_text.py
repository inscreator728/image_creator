    ...
    def run(self):
        try:
            ...
            if self.mode == "Text":
                ...
                try:
                    if not self.custom_text or not os.path.exists(self.custom_text):
                        self.error.emit("Excel file not found."); return
                    import pandas as pd
                    df = pd.read_excel(self.custom_text)
                    if len(df) <= 1:
                        self.error.emit("Excel file has no data rows."); return
                    count_values = df.iloc[1:,0].dropna().astype(str).str.strip().tolist()
                except Exception as e:
                    self.error.emit(f"Excel read error: {e}"); return
            else:
            ...
            for val in count_values:
                ...
                base_filename = safe_val if safe_val else f"item_{processed+1}"
                final_name = base_filename
                counter = 1
                while os.path.exists(os.path.join(self.output_folder, f"{final_name}.{self.out_ext}")):
                    final_name = f"{base_filename}_{counter}"
                    counter += 1
                on = f"{final_name}.{self.out_ext}"
                op = os.path.join(self.output_folder, on)
...
class CreatorApp(QMainWindow):
    def __init__(self):
        ...
        # Counter / text box
        counter_box = QGroupBox("Excel Import")
        self.excel_btn = QPushButton("Load Excel File")
        self.excel_path_line = QLineEdit()
        self.excel_path_line.setReadOnly(True)
        self.excel_btn.clicked.connect(self.load_excel)
        counter_layout = QFormLayout()
        counter_layout.addRow(self.excel_btn, self.excel_path_line)
    ...
    def load_excel(self):
        f, _ = QFileDialog.getOpenFileName(self, 'Open Excel file', '', 'Excel Files (*.xlsx *.xls)')
        if not f:
            return
        self.excel_path_line.setText(f)
        self.log_msg(f'Excel file: {f}')
    ...
    def _collect_params(self):
        ...
        mode = "Excel"
        custom_text = self.excel_path_line.text() or ""
    ...
    def update_preview(self):
        ...
        if mode == "Excel":
            text = ""
            try:
                if custom_text and os.path.exists(custom_text):
                    import pandas as pd
                    df = pd.read_excel(custom_text)
                    if len(df) > 1:
                        text = str(df.iloc[1,0]).strip()
            except Exception:
                text = ""
        else:
